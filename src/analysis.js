...
const analysis= async ()=> {
  ...
  let driversPromise;
  try {
   driversPromise = await Promise.allSettled(arrayOfGetDriverX)
  } catch (error) {
    console.log(error)
  }
  ...
  let mostTripsDriverDetails;
  try {
    mostTripsDriverDetails = await(getDriver(mostTripsDriver.driverId))
  } catch (error) {
    console.log(error)
  }
  
  result['mostTripsByDriver']= getDriverDetails(mostTripsDriverDetails, mostTripsDriver.driverId,mostTripsDriver.noOfTrips, amountsEarned);
  highestEarner= getHighestEarningDriver(amountsEarned);
  let tripsObj= getTripsObj(drivers);
  let mostEarnedDriverDetails;
  try {
    mostEarnedDriverDetails= await(getDriver(highestEarner.driverId));
  } catch (error) {
    console.log(error)
  }
  result['highestEarningDriver']= getDriverDetails(mostEarnedDriverDetails, highestEarner.driverId, tripsObj[highestEarner.driverId], 0)
